...
public class M12gauEntity extends GeoVehicleEntity {
    
    // 炮台旋转数据
    public static final EntityDataAccessor<Float> TURRET_Y_ROT = SynchedEntityData.defineId(M12gauEntity.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> TURRET_X_ROT = SynchedEntityData.defineId(M12gauEntity.class, EntityDataSerializers.FLOAT);
    
    // 添加上一tick的角度用于插值
    public float turretYRotO;
    public float turretXRotO;

    public M12gauEntity(EntityType<? extends M12gauEntity> type, Level world) {
        super(type, world);
    }
    ...
    private PlayState shootPredicate(AnimationState<M12gauEntity> event) {
        // 添加调试输出
        System.out.println("[M12gau DEBUG] shootPredicate called");
        
        // 检查主炮是否正在射击 - 尝试两种方式
        int timerBySeat = getShootAnimationTimer(2, 0);
        int timerByName = getShootAnimationTimer("MainGun");
        
        System.out.println("[M12gau DEBUG] Shoot animation timer for seat 2, weapon 0: " + timerBySeat);
        System.out.println("[M12gau DEBUG] Shoot animation timer for weapon 'MainGun': " + timerByName);
        
        // 使用较大的计时器值
        int timer = Math.max(timerBySeat, timerByName);
        System.out.println("[M12gau DEBUG] Using timer value: " + timer);
        
        if (timer > 0) {
            System.out.println("[M12gau DEBUG] Playing fire animation, timer = " + timer);
            // 使用动画文件中的实际动画名称："fire"
            // 使用thenLoop而不是thenPlay，使动画在射击期间持续循环播放
            return event.setAndContinue(RawAnimation.begin().thenLoop("fire"));
        }
        
        System.out.println("[M12gau DEBUG] Playing idle animation");
        // 使用动画文件中的实际动画名称："idle"
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar data) {
        // 添加调试输出
        System.out.println("[M12gau DEBUG] registerControllers called");
        
        // 注册射击动画控制器
        data.add(new AnimationController<>(this, "shoot", 0, this::shootPredicate));
        
        System.out.println("[M12gau DEBUG] Animation controller registered");
    }
    ...
    public void hitBlock(Vec3 pos, GunData gunData, Entity shooter) {
        if (level() instanceof ServerLevel serverLevel) {
            if (gunData.compute().explosionRadius > 0) {
                // 如果有爆炸半径，生成更多粒子
                ParticleTool.sendParticle(serverLevel, ParticleTypes.END_ROD, 
                    pos.x, pos.y, pos.z, 24, 0, 0, 0, 0.2, true);
                ParticleTool.sendParticle(serverLevel, ParticleTypes.LAVA, 
                    pos.x, pos.y, pos.z, 8, 0, 0, 0, 0.4, true);
            } else {
                // 普通命中粒子
                ParticleTool.sendParticle(serverLevel, ParticleTypes.END_ROD, 
                    pos.x, pos.y, pos.z, 4, 0, 0, 0, 0.05, true);
                ParticleTool.sendParticle(serverLevel, ParticleTypes.LAVA, 
                    pos.x, pos.y, pos.z, 2, 0, 0, 0, 0.15, true);
            }
        }
    }
