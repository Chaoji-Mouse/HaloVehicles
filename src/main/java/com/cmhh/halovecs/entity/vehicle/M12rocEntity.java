...
public class M12rocEntity extends GeoVehicleEntity {
    
    // 炮台旋转数据
    public static final EntityDataAccessor<Float> TURRET_Y_ROT = SynchedEntityData.defineId(M12rocEntity.class, EntityDataSerializers.FLOAT);
    public static final EntityDataAccessor<Float> TURRET_X_ROT = SynchedEntityData.defineId(M12rocEntity.class, EntityDataSerializers.FLOAT);
    
    // 添加上一tick的角度用于插值
    public float turretYRotO;
    public float turretXRotO;

    public M12rocEntity(EntityType<M12rocEntity> type, Level world) {
        super(type, world);
    }
    ...
    private PlayState shootPredicate(AnimationState<M12rocEntity> event) {
        // 添加调试输出
        System.out.println("[M12roc DEBUG] shootPredicate called");
        
        // 检查主炮是否正在射击 - 尝试两种方式
        int timerBySeat = getShootAnimationTimer(2, 0);
        int timerByName = getShootAnimationTimer("MainGun");
        
        System.out.println("[M12roc DEBUG] Shoot animation timer for seat 2, weapon 0: " + timerBySeat);
        System.out.println("[M12roc DEBUG] Shoot animation timer for weapon 'MainGun': " + timerByName);
        
        // 使用较大的计时器值
        int timer = Math.max(timerBySeat, timerByName);
        System.out.println("[M12roc DEBUG] Using timer value: " + timer);
        
        if (timer > 0) {
            System.out.println("[M12roc DEBUG] Playing fire animation, timer = " + timer);
            // 使用动画文件中的实际动画名称："fire"
            // 使用thenLoop而不是thenPlay，使动画在射击期间持续循环播放
            return event.setAndContinue(RawAnimation.begin().thenLoop("fire"));
        }
        
        System.out.println("[M12roc DEBUG] Playing idle animation");
        // 使用动画文件中的实际动画名称："idle"
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar data) {
        // 添加调试输出
        System.out.println("[M12roc DEBUG] registerControllers called");
        
        // 注册射击动画控制器
        data.add(new AnimationController<>(this, "shoot", 0, this::shootPredicate));
        
        System.out.println("[M12roc DEBUG] Animation controller registered");
    }
